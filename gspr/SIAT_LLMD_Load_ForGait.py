...
# path setup
path_data_set_folder_global = "D:/ly/sEMG/SIAT-Lower-Limb-Motion-Dataset-Codes-main/SIAT_LLMD20230404/" 
path_save_results_to_folder = "D:/ly/sEMG/SIAT-Lower-Limb-Motion-Dataset-Codes-main/SIAT_LLMD20230404/Code/codeV4.2/result/" 
        
path_input_data_folder = 'D:/ly/sEMG/SIAT-Lower-Limb-Motion-Dataset-Codes-main/SIAT_LLMD20230404/Code/codeV4.2/result/emg_filtered/20241012094207/'  
...
# channels_with_rows: 1, 0, default:1 所有处理
...
def get_all_raw_data_path(path_data_set_folder, subjects_list, movements_list):
    ...
    for subjects_num in range(subjects_quantity):
        if subjects_list[subjects_num] < 10:
            sl_list = "0" + str(subjects_list[subjects_num])
        else:
            sl_list = str(subjects_list[subjects_num])

        path_subject_raw_label_folder = path_data_set_folder + "Sub" + sl_list + "/Labels/"
        path_subject_raw_data_folder = path_data_set_folder + "Sub" + sl_list + "/Data/"
        for movements_num in range(movements_quantity):
            path_raw_label[subjects_num, movements_num] = path_subject_raw_label_folder + get_FileFullName_in_folder_with_SameString_and_extension(path_folder=path_subject_raw_label_folder, same_string=movements_list[movements_num],extension='.xlsx')
            ...
            # path_raw_data[subjects_num, movements_num] = path_subject_raw_data_folder + get_FileFullName_in_folder_with_SameString_and_extension(path_folder=path_subject_raw_data_folder, same_string=movements_list[movements_num], extension='.csv')
    # print('lhy 24.10.11')
...
def get_label_from_target_list(target_list=[], label_type='all'):
    ...
    data_num_label_list = LabelEncoder().fit_transform(values) # 拟合转换数字
    data_OneHot_label_list = OneHotEncoder(sparse=False).fit_transform(data_num_label_list.reshape((-1, 1))) # 拟合转换独热编码
    if label_type == 'num':
...
def read_excel(path_excel='None', read_locations_begin=[], read_locations_end=[], read_locations_sheet=[]):
    ...
    # workbook = pd.read_csv(filename=path_excel)
    # print('lhy 24.10.11')
...
def get_emg_windows_from_one_data(data_path):
    ...
    label_status = np.int32(list(data['label_status'][0]))
